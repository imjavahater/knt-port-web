...
},{}],"3aYVp":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "4818c8a6e881677d";
...
},{}],"daUln":[function(require,module,exports) {
...
function resizeCanvas() {
    ...
    if (options.canvasGradient) {
        const gradient = ctxBackground.createLinearGradient(0, 0, canvasBackground.width, canvasBackground.height);
        ...
        const img = new Image();
        img.onload = function() {
            ctxBackground.drawImage(img, 0, 0, canvasBackground.width, canvasBackground.height);
...
function Star(x, y) {
    ...
    if (options.percentStarsConnecting === 100) this.connects = true;
    else this.connects = options.connectionsWhenNoMouse && Math.random() < options.percentStarsConnecting / 100;
...
function animateStars() {
    ...
    stars.forEach((star)=>{
        ...
        if (options.bounceOnEdges) {
            if (star.x > canvasStars.width || star.x < 0) star.speedX = -star.speedX;
            if (star.y > canvasStars.height || star.y < 0) star.speedY = -star.speedY;
        } else {
        ...
        for(let i = -1; i <= 1; i++)for(let j = -1; j <= 1; j++){
            ...
            if (cells[neighbourCellX] && cells[neighbourCellX][neighbourCellY]) cells[neighbourCellX][neighbourCellY].forEach((otherStar)=>{
                ...
                if (distance < options.maxDistance && (mouseDistance < options.mouseRadius || star.connects && otherStar.connects)) {
                    ...
                    ctxStars.strokeStyle = options.connectionColor.replace("${opacity}", opacity.toString());
                    ctxStars.lineWidth = options.lineThickness;
                    if (options.connectionLinesDashed) ctxStars.setLineDash(options.dashedLinesConfig);
                    else ctxStars.setLineDash([]);
                    ctxStars.stroke();
                }
            });
        }
...
function createStars() {
    resizeCanvas();
    const numberOfStars = starDensities[options.starDensity] * canvasStars.width * canvasStars.height;
...
},{}]},["1xC6H","3aYVp","daUln"], "daUln", "parcelRequire6158")

//# sourceMappingURL=index.e881677d.js.map
